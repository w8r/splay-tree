...
(function (global, factory) {
  ...
  function remove (i, root, comparator, tree) {
    ...
    var parent = null;
    while (current) {
      var cmp = comparator(i, current.key);
      if (cmp === 0) {
  ...
  function findParent (t, root, comparator) {
    ...
    var parent = null;
    while (current) {
      var cmp = comparator(t.key, current.key);
      if (cmp === 0) {
        break;
      } else if (cmp < 0) {
        parent = current;
  ...
  function removeNode(t, parent, comparator, tree) {
    ...
    if (t.right === null || t.left === null) {
      ...
      if (parent$1.right === child) {
        parent$1.right = child.left;
      } else {
        parent$1.left = child.left;
      }
   ...
   * @return boolean true if node removed.
   */
  Tree.prototype.removeNode = function removeNode$1 (node, parent) {
  ...
  Tree.prototype.pop = function pop () {
    ...
    if (node) {
      ...
      var result = { key: node.key, data: node.data };
   ...
   * @return {Node|null}
   */
  Tree.prototype.findParent = function findParent$1 (node) {
